...
private _treatmentType = switch (true) do {
	case ("_250_" in _fluidUsed): {"_250"}; //_250_ ensures it doesn't get conflicts with ID
	case ("_500_" in _fluidUsed): {"_500"};
	default {""};
};

_treatmentTypeJoined = "BloodIV" + _treatmentType;
...
if (GVAR(KAMLoaded)) then {
	...
	_treatmentTypeJoined = "BloodIV" + _kamSpecific + _treatmentType;
	private _isCompatible = [_patient getVariable "KJW_MedicalExpansion_Core_bloodType",_type] call KJW_MedicalExpansion_Core_fnc_checkBloodTypeCompatibility;
	if !_isCompatible then {
		[_treatmentTypeJoined, _patient] call kat_circulation_fnc_treatmentAdvanced_IV;
	};
};

[_medic, _patient, _bodyPart, _treatmentTypeJoined, objNull, _fluidInfo#0, _fluidUsed] call ace_medical_treatment_fnc_ivBag;
